...
type KommoConfig = {
  ...
  messagingGatewayUrl?: string;      // Optional: Messaging Gateway URL
  messagingGatewayApiKey?: string;   // Optional: Messaging Gateway API Key
};
```

### Optional: Messaging Gateway Configuration

If you're using the Messaging Gateway for WhatsApp/Telegram/Instagram messaging:

```typescript
const config: KommoConfig = {
  baseUrl: 'https://your-domain.kommo.com',
  accessToken: 'your-oauth2-access-token',
  messagingGatewayUrl: 'https://gateway.syncra.com',
  messagingGatewayApiKey: 'sk_live_abc123...',
};
```

**Note:** The Messaging Gateway is **multi-tenant**. Each client must have their own API Key registered in the Gateway. The Gateway automatically identifies the client from the API Key, so you should **not include `client_id` in request bodies**.
  ...
  "mcpServers": {
    "kommo-crm": {
      ...
      "env": {
        ...
        "SYNC_MESSAGING_URL": "https://gateway.syncra.com",
        "SYNC_MESSAGING_API_KEY": "sk_live_abc123..."
      }
    }
  }
}
...
**Note:** `SYNC_MESSAGING_URL` and `SYNC_MESSAGING_API_KEY` are optional. Only include them if you're using the Messaging Gateway.
...
```typescript
import { listPipelines, type KommoConfig } from '@syncraengine/kommo-mcp';
